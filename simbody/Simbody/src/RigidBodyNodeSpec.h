...
// EU Velocity calculated acording to modal vel
void realizeModalVelocity(const SBArticulatedBodyInertiaCache& abc, 
                          const SBStateDigest&                 sbs,
                          SimTK::Vector& ModalV) const 
{
    const SBModelVars&          mv = sbs.getModelVars();
    const SBTreePositionCache&  pc = sbs.getTreePositionCache();
    SBTreeVelocityCache&        vc = sbs.updTreeVelocityCache();
    ...
    // EU Additional quantitie needed
    const HType&              G = getG(abc);
    const Mat<dof,dof>&       DI  = getDI(abc);
    //Take sqrt(DI)
    Mat<dof,dof> sqrtDI = DI;
    for(int i=0; i<dof; i++){
        sqrtDI(i,i) = std::sqrt(DI(i,i));
    }
    //const Vec<dof>& ModalU = (sqrtDI*ModalV) - (~G * (getSpatialVel(vc)));
    Vec<dof> ModalVVec; // TODO recast Vector to Vec<>
    for(int i=0; i<dof; i++){
      ModalVVec(i) = ModalV(i);
    }
    const Vec<dof>& ModalU = (sqrtDI*ModalVVec) - (~G * (getSpatialVel(vc)));

    // Mobilizer specific.
    calcQDot(sbs, &allU[uIndex], &sbs.updQDot()[qIndex]);

    //updV_FM(vc)    = getH_FM(pc) * u;   // 6*dof flops // RESTORE
    //updV_PB_G(vc)  = getH(pc)    * u;   // 6*dof flops // RESTORE
    updV_FM(vc)    = getH_FM(pc) * ModalU;   // EU
    updV_PB_G(vc)  = getH(pc)    * ModalU;   // EU
