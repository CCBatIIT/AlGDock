...
cpdef calc_desolvationGrid(int_t[:,:,:] receptor_MS_grid, \
  ...
  while i < counts[0]:
    ...
    while j < counts[1]:
      ...
      # TODO: Filter receptor_SAS_points earlier in the loop
      n = 0
      while n < nreceptor_SAS_points:
        if (receptor_SAS_points[n,0]<SAS_x_min) or \
           (receptor_SAS_points[n,0]>SAS_x_max) or \
           (receptor_SAS_points[n,1]<SAS_y_min) or \
           (receptor_SAS_points[n,1]>SAS_y_max) or \
           (receptor_SAS_points[n,2]<SAS_z_min) or \
        ...
        if (dx*dx + dy*dy + dz*dz)<ligand_atom_radius2:
          newly_inaccessible_SAS_points.append(\
            (SAS_point_x,SAS_point_y,SAS_point_z))
        n += 1
      ...
      if n_newly_inaccessible_SAS_points==0:
        ...
        while sphere_i < nsphere_points:
          ...
          # TODO: Filter receptor atoms
          while atom_j < nreceptor_atoms:
        ...
        # Decrement of the marks of grid points within the probe radius
        n = 0
