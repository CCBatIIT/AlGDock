...
class SimulationData:
  ...
  def _load_pkl_gz(self):
    ...
    if self.process == 'CD' and self.pose > -1:
      ...
      progress_FN = os.path.join(self.dir, '%s_progress.pkl.gz' % (self.process))
    ...
    if (saved['progress'] is None) or (saved['data'] is None):
      ...
      if self.process == 'CD' and self.pose > -1:
        progress_FN = os.path.join(
            self.dir, '%s_progress_self.pose%03d.pkl.gz.BAK' % (self.process, self.pose))
        data_FN = os.path.join(self.dir, '%s_data_self.pose%03d.pkl.gz.BAK' % (self.process, self.pose))
      else:
        progress_FN = os.path.join(self.dir, '%s_progress.pkl.gz.BAK' % (self.process))
