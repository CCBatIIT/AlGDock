...
last modified {2}
    ...
    def _setup_universe(self, do_dock=True):
        ...
        self.molecule.heavy_atoms = [ind for (atm, ind) in
                                     zip(self.molecule.atoms, range(self.molecule.numberOfAtoms())) if
                                     atm.type.name != 'hydrogen']
    ...
    """
        ...
        while (not self.cool_protocol[-1]['crossed']):
            ...
            if mean_acc < self.params['cool']['min_repX_acc']:
                ...
                self.tee(
                    "  rejected new state, as estimated replica exchange acceptance rate of %f is too low" % mean_acc)
                ...
                self.tee(
                    "  rejected previous state, as estimated replica exchange acceptance rate of %f is too high" % mean_acc)
            else:
    ...
    """
        ...
        if self.dock_protocol == []:
            ...
            if not (hasattr(self, '_random_trans') and hasattr(self, '_random_rotT')):
                ...
                self._n_trans = max(
                    min(np.int(np.ceil(self._forceFields['site'].volume * self.params['dock']['site_density'])),
                        self._max_n_trans), 5)
        ...
        while (not self.dock_protocol[-1]['crossed']):
            ...
            if len(self.dock_protocol) == 2:  # Cooling state 0 configurations, randomly oriented
                ...
                for ind in seedIndicies:
                    (c, i_rot, i_trans) = np.unravel_index(ind, (
                    self.params['dock']['seeds_per_state'], self._n_rot, self._n_trans))
                    seeds.append(np.add(np.dot(cool0_confs[c], self._random_rotT[i_rot, :, :]),
                                        self._random_trans[i_trans].array))
                confs = None
                E = {}
    ...
    """
        ...
        for c in range(len(self.f_grid), self._dock_cycle):
            ...
            self.tee("  calculated %s binding PMF of %f RT using cycles %d to %d" % ( \
                self.params['dock']['phases'][-1], B[self.params['dock']['phases'][-1] + '_MBAR_based'], fromCycle,
                toCycle - 1))
            updated = True
    ...
    """
        ...
        w = np.exp(
            (-1 / RT_TARGET + 1 / RT_HIGH) * np.reshape(E['MM'] - min(E['MM']), (-1, self._n_rot, self._n_trans)))
        ...
        for r in range(len(r_range)):
            for t in range(len(t_range)):
                ...
                du = Psi_c / RT_TARGET
                min_du = min(du)
                random_dock_stats['f_grid'][r][t] = -RT_TARGET * (
                np.log(np.sum(w_c * np.exp(-du + min_du)) / np.sum(w_c)) + min_du)
        ...
        for scalable in self._scalables:
            if (scalable in lambda_n.keys()) and lambda_n[scalable] > 0:
                ...
                if not scalable in self._forceFields.keys():
                    ...
                    grid_FN = self._FNs['grids'][
                        {'sLJr': 'LJr', 'sLJa': 'LJa', 'LJr': 'LJr', 'LJa': 'LJa', 'ELE': 'ELE'}[scalable]]
                    grid_scaling_factor = 'scaling_factor_' + \
                                          {'sLJr': 'LJr', 'sLJa': 'LJa', 'LJr': 'LJr', 'LJa': 'LJa',
                                           'ELE': 'electrostatic'}[scalable]
                    ...
                    if scalable == 'LJr':
                        ...
                        self._forceFields[scalable] = TrilinearISqrtGridForceField(grid_FN,
                                                                                   grid_conversion * lambda_n[scalable],
                                                                                   grid_scaling_factor,
                    ...
                    self.tee('  %s grid%s loaded from %s in %s' % (
                    scalable, apbs_grid, grid_FN, HMStime(time.time() - loading_start_time)))
...
###########################################
            ...
            #print "I have " + str(multiprocessing.cpu_count()) + " cores"
...
############################################################################################
            if process == 'dock':
                ...
                if self.params['dock']['rmsd'] is not False:
                    E['rmsd'] = np.array([np.sqrt(((confs[k][self.molecule.heavy_atoms, :] - \
                                                    self.confs['rmsd']) ** 2).sum() / self.molecule.nhatoms) for k in
                                          range(K)])
        ...
        for state in range(K):
            ...
            for term in terms:
                E_state[term] = np.array(
                    [storage['energies'][store_indicies[snap]][term][inv_state_inds[snap][state]] for snap in
                     range(nsaved)])
            Es.append([E_state])
        ...
        if ((not self.params[process]['no_protocol_refinement']) and \
            ...
            if insert_state.any():
                for k in reversed(range(K - 1)):
                    if insert_state[k]:
                        self.tee("  due to exchange acceptance probability of %f, inserted state between %d and %d" % (
                        mean_acc[k], k, k + 1))
    ...
    """
        ...
        if (self.params[process]['repX_cycles'] is not None) and \
                ((getattr(self, '_%s_total_cycle' % process) < self.params[process]['repX_cycles']) or (
                    getattr(self, '_%s_cycle' % process) == 0)):
            ...
            while ((getattr(self, '_%s_total_cycle' % process) < self.params[process]['repX_cycles']) or (
                getattr(self, '_%s_cycle' % process) == 0)):
        ...
        if process == 'cool':
            ...
            if not (((getattr(self, '_%s_total_cycle' % process) < self.params[process]['repX_cycles']) or (
                getattr(self, '_%s_cycle' % process) == 0))):
                self._postprocess()
                self.calc_f_RL()
        self._clear_lock(process)
    ...
    """
        ...
        if self.params['dock']['score'] is True:
            ...
            self.tee("  {dock6} from dock6, {initial_dock} from initial docking, and {duplicated} duplicated".format(
                **count))
        self._clear_lock('dock')
    ...
    """
        ...
        for k in range(K - 1):
            ...
            try:
                f_k_BAR[k + 1] = pymbar.BAR(w_F, w_R, relative_tolerance=0.000001, verbose=False,
...
''')
        ...
        dat.pop(0)
        if len(dat) > 0:
            E = np.array([rec[:rec.find('\nminimization')].replace('1-4 ', '1-4').split()[1::3] for rec in dat],
    ...
    """
        ...
        if includeReceptor and includeLigand:
            for array in confs:
                array = factor * array
                x = N.concatenate(
                    (receptor_x0, N.take(array[:, 0], self.molecule.prmtop_atom_order), receptor_x1)).astype(N.Float16)
                y = N.concatenate(
                    (receptor_y0, N.take(array[:, 1], self.molecule.prmtop_atom_order), receptor_y1)).astype(N.Float16)
                z = N.concatenate(
                    (receptor_z0, N.take(array[:, 2], self.molecule.prmtop_atom_order), receptor_z1)).astype(N.Float16)
    ...
    def sample_with_each_state(self,
        ...
        if my_process == 'dock' and my_params['dock']['MCMC_moves'] > 0 and my_lambdas[my_state_inds[my_k]['a']] < 0.1:
            ...
            output_dic['E_acc_MC_k'] = self._MC_translate_rotate(25) / 25
            output_dic['my_MC_time'] = my_MC_time + (time.time() - my_MC_start_time)
        (confs_k, potEs_k, Hs_k, delta_t) = my_sampler[my_process](steps=my_params[my_process]['steps_per_sweep'], \
                                                                T=my_T, delta_t=my_delta_t, \
                                                                normalize=(my_process == 'cool'), adapt=False)
        output_dic['confs_k'] = np.copy(confs_k[-1])

        if my_process == 'cool':
            output_dic['E_MM_k'] = potEs_k[-1]
        output_dic['Ht_k'] = my_Ht[my_k] + Hs_k

        q.put(output_dic)  # put this result into result_queue for the main process to use
