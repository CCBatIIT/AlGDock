...
# TO DO: Make the soft core grid softer by reducing grid scaling factors?
# TO DO: Initiation as a reservoir for replica exchange
# TO DO: Store data in fewer files
...
last modified {2}
    ...
    args['default_dock'] = dict(args['default_cool'].items() + {
      ...
      'rmsd':False,
      'score':False}.items() + \
      [('receptor_'+phase,None) for phase in phases])
      ...
      for key in args['default_'+p].keys():
        ...
        if (specific_key in namespace.keys()) and \
          ...
          args['new_'+p][key] = namespace[specific_key]
                     ...
                     ['receptor_'+phase for phase in phases]) and \
  ...
  def initial_cool(self):
    ...
    (2) starting configurations for replica exchange (cool_replicas.mdcrd.gz)
    """
    ...
    while (not self.cool_protocol[-1]['crossed']):
      ...
      if mean_acc<self.params['cool']['min_repX_acc']:
        ...
        self.confs['initial_cool'].append(confs)
    ...
    for state in range(K):
      if self.params['cool']['keep_intermediate'] or \
          (state==0) or (state==(K-1)):
        self._write_mdcrd(\
          join(self.dir['cool'],'cool%d_0.mdcrd.gz'%(state)), \
          self.confs['initial_cool'][state], title='state=%d,cycle=0'%(state))

    postprocess_start_time = time.time()
    self._cool_cycle += 1
    self._postprocess([('cool',-1,-1,'L')])

    self._write_mdcrd(join(self.dir['cool'],'cool_replicas.mdcrd.gz'),
      self.confs['cool_replicas'], title='cool_replicas,cycle=-1')
    self._write_pkl_gz(join(self.dir['cool'],'cool_protocol.pkl.gz'), \
      self.cool_protocol)

    self._write_pkl_gz(join(self.dir['cool'],'initial_cool.pkl.gz'),\
      (self.cool_protocol,self.confs['cool_replicas'],\
       self.confs['initial_cool'],self.cool_Es))
  ...
  def calc_f_L(self, readOnly=False, redoSolvation=False):
    ...
    for c in range(len(self.f_L_solvation), self._cool_cycle):
      ...
      u_F = {}
      for key in ['L'+phase for phase in phases]+['MM']:
      ...
      self.tee("  calculated GBSA solvation free energy of %f RT "%(\
                  self.f_L_solvation[-1]['GBSA'])+\
  ...
  def initial_dock(self, randomOnly=False):
    """
      ...
      (2) starting configurations for replica exchange (dock_replicas.mdcrd.gz)
    """
    ...
    checkpoint = self._load_pkl_gz(join(self.dir['dock'],'initial_dock.pkl.gz'))
    ...
    while (checkpoint is not None) or (not self.dock_protocol[-1]['crossed']):
      ...
      if checkpoint is None:
        ...
        if len(self.dock_protocol)==2: # Cooling state 0 configurations, randomly oriented
          ...
          self.confs['initial_dock'] = [[repX_conf]]
        ...
        # Save checkpoint
        self._write_pkl_gz(join(self.dir['dock'],'initial_dock.pkl.gz'),\
          (self.dock_protocol,seeds,self.confs['dock_replicas'],\
           self.confs['initial_dock'],self.dock_Es))
        ...
        (self.dock_protocol,seeds,self.confs['dock_replicas'],\
           self.confs['initial_dock'],self.dock_Es) = checkpoint
        if len(self.confs['initial_dock'])>0:
          confs = self.confs['initial_dock'][-1]
          E = self.dock_Es[-1]
        else:
          confs = None
          E = {}
      ...
      # Store data
      self.tee('  generated %d configurations with progress variable %f in %f s'%(len(confs), self.dock_protocol[-1]['a'], time.time()-sim_start_time))
      ...
      if len(self.dock_protocol)>2:
        ...
        if (mean_acc<self.params['dock']['min_repX_acc']):
          ...
          self.confs['initial_dock'].append(confs)
    ...
    self.tee("  there are %d states in the docking process"%(K))
      
    # Save data
    for state in range(K):
      if self.params['dock']['keep_intermediate'] or (state==(K-1)):
        self._write_mdcrd(\
          join(self.dir['dock'],'dock%d_0.mdcrd.gz'%(state)), \
          self.confs['initial_dock'][-1], title='state=%d,cycle=1'%state)
    ...
    self._write_mdcrd(join(self.dir['dock'], 'dock_replicas.mdcrd.gz'), \
      self.confs['dock_replicas'], title='dock_replicas,cycle=0')
  ...
  def calc_f_RL(self, readOnly=False, redo=True):
    """
  ...
  def view_samples(self, process='dock', state=None,
    ...
    for c in range(firstCycle,getattr(self,'_%s_cycle'%process)):
      if not isfile(join(self.dir[process],process+'%d_%d.dcd'%(state,c))):
        crd = self._read_mdcrd(join(self.dir[process],process+'%d_%d.mdcrd.gz'%(state,c)), split=True)
    ...
    for scalable in self._scalables:
      if (scalable in lambda_n.keys()) and lambda_n[scalable]>0:
        ...
        if not scalable in self._forceFields.keys():
          ...
          self.tee('  %s grid %s loaded from %s in %f s'%(scalable, apbs_grid, grid_FN, time.time()-loading_start_time))
  ...
  def _replica_exchange(self, process):
    ...
    for sweep in range(self.params[process]['sweeps_per_cycle']):
      ...
      for k in range(K):
        ...
        if process=='dock' and self.params['dock']['MCMC_moves']>0 and state_inds[k]<7:
    ...
    if cycle==0:
      ...
      if insert_state.any():
        ...
        self._write_pkl_gz(join(self.dir[process],process+'_protocol.pkl.gz'),
          lambdas)
        self._write_mdcrd(join(self.dir[process],process+'_replicas.mdcrd.gz'), \
          self.confs[process+'_replicas'], \
    ...
    for state in range(K):
      if self.params[process]['keep_intermediate'] or \
        ...
        self._write_mdcrd(\
          join(self.dir[process],process+'%d_%d.mdcrd.gz'%(state,cycle)), \
  ...
  def _postprocess(self,
    ...
    for (p, state, cycle, moiety) in conditions:
      ...
      for c in cycles:
        ...
        for phase in phases:
          ...
          label = moiety+phase
          crd_FN = join(p_dir,'%s.mdcrd.gz'%(prefix))
          ...
          if (not (redo_dock and p=='dock')) and \
            (readOnly \
            or (not isfile(crd_FN) and p!= 'original') \
            or (p == 'original' and \
    ...
    for phase in phases:
      if (self.params['dock']['receptor_'+phase] is None) and \
        ...
        self.params['dock']['receptor_'+phase] = self.original_Es[0][0]['R'+phase][-1]
        self.store_params('dock')

    # Save progress
    for (p,c) in updated:
      self.tee("  postprocessed {0}ing cycle {1} with AMBER and/or NAMD".format(p,c))
      if p=='cool' or p=='dock':
        energyFN = join(self.dir[p], '%s_Es.pkl.gz'%(p))
        self._write_pkl_gz(energyFN, getattr(self, p+'_Es'))
...
''')
  ...
  def _cool0_storage(self, save=False):
    ...
    if save or (not isfile(cool0_store_FN)):
      ...
      confs = []
      c = 0
      while isfile(join(self.dir['cool'],'cool0_%d.mdcrd.gz'%c)):
        confs += self._read_mdcrd(join(self.dir['cool'],'cool0_%d.mdcrd.gz'%c))
        c += 1
      # Load energies
      E_MM = []
  ...
  def _read_crd(self, crdFN, split=True, reorder=False, return_title=False):
    """ 
  ...
  def _write_crd(self, crdFN, crd, title='', \
      appendIfExists=False,
      use_prmtop_atom_order=False,
      vmdStyle=False):
    """
    Writes an AMBER format coordinate/restart file
    """
    if (appendIfExists and isfile(crdFN)):
      ...
      if crdFN.endswith('.gz'):
        ...
        crdF = open(crdFN,'w')
      # Write the header
      crdF.write({False:'',True:'prmtop_atom_order_'}[use_prmtop_atom_order]+ \
        title+'\n') # Title
      if not vmdStyle:
        crdF.write('%d\n'%self.universe.numberOfAtoms())

    if use_prmtop_atom_order:
      if isinstance(crd,list):
        ordered = np.vstack([c[self.molecule.prmtop_atom_order,:] \
          for c in crd])/MMTK.Units.Ang
      else:
        ordered = crd[self.molecule.prmtop_atom_order,:]/MMTK.Units.Ang
    else:
      ordered = np.vstack(crd)/MMTK.Units.Ang
  ...
  def _read_mdcrd(self, mdcrdFN, split=True, reorder=True, return_title=False):
    """ 
    Reads an AMBER format trajectory file.
    """
    if not isfile(mdcrdFN):
      raise Exception('Coordinate file %s does not exist!'%mdcrdFN)
    if mdcrdFN.endswith('.gz'):
      mdcrdF = gzip.open(mdcrdFN,'r')
    else:
      mdcrdF = open(mdcrdFN,'r')
    dat = mdcrdF.read().strip().split('\n')
    mdcrdF.close()
    ...
    if split:
      crd = np.vsplit(crd,crd.shape[0]/self.universe.numberOfAtoms())
      if reorder:
        crd=[c[self.molecule.inv_prmtop_atom_order,:] for c in crd]
      print "  read %d configurations from %s"%(len(crd), mdcrdFN)
    elif reorder:
      crd=crd[self.molecule.inv_prmtop_atom_order,:]
  ...
  def _write_mdcrd(self, mdcrd_FN, confs, title='', \
    ...
    if isfile(mdcrd_FN):
      os.rename(mdcrd_FN,mdcrd_FN+'.BAK')
    if mdcrd_FN.endswith('.gz'):
      mdcrdF = gzip.open(mdcrd_FN,'w')
    else:
      mdcrdF = open(mdcrd_FN,'w')
    # Write the header
    mdcrdF.write(title+'\n') # Title
    ...
    for conf in confs:
      flattened = conf.flatten()
      for start_ind in range(0,len(flattened),10):
        mdcrdF.write(''.join(['%8.3f'%c for c in flattened[start_ind:start_ind+10]]) + '\n')
    mdcrdF.close()
  ...
  def _load_progress(self, p):
    ...
    if (not hasattr(self, p+'_Es')) or (getattr(self, p+'_Es')==[]):
      setattr(self, p+'_Es', \
        self._load_pkl_gz(join(self.dir[p], '%s_Es.pkl.gz'%(p))))
      cycle = len(getattr(self, p+'_Es')[0])

    # Load replicas
    if not p+'_replicas' in self.confs.keys():
      replica_FN = join(self.dir[p], p+'_replicas.mdcrd.gz')
      if isfile(replica_FN):
        (confs, title) = self._read_mdcrd(replica_FN, return_title=True)
