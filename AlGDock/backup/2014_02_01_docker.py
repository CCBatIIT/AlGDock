...
Molecular docking with adaptively scaled alchemical interaction grids
  ...
  def initial_cool(self):
    ...
    self.cool_protocol = [{'MM':True, 'T':T_HIGH, \
                          'delta_t':1.0*MMTK.Units.fs, 'crossed':False}]
    ...
    for T in np.linspace(0.,T_HIGH,33)[1:]:
      self.sampler['cool'](steps = 500, T=T, delta_t=self.delta_t)
    ...
    # Main loop for initial cooling: choose new temperature, randomly select seeds, simulate
    T = T_HIGH
    ...
    self._write_pkl_gz(join(self.dir['cool'],'cool_protocol.pkl.gz'), self.cool_protocol)

    self._clear_lock('cool')
    print "\nElapsed time for initial cooling: %f s"%(time.time()-cool_start_time)
  ...
  def calc_f_L(self, readOnly=False):
    ...
    if exists(f_L_FN):
      (self.f_L_solvation, self.f_cool, self.cool_equilibrated_cycle, self.cool_mean_acc) = self._load_pkl_gz(f_L_FN)
    else:
    ...
    for c in range(len(self.f_L_solvation), self._cool_cycle):
      ...
      print "  calculated cooling free energy of %f kT using cycles %d to %d"%(self.f_cool[-1][-1], fromCycle, toCycle-1)
      updated = True

    if updated:
      self._write_pkl_gz(f_L_FN, (self.f_L_solvation, self.f_cool, self.cool_equilibrated_cycle, self.cool_mean_acc))
  ...
  def initial_dock(self):
    ...
    if self.cool_protocol==[]:
      raise Exception('Cannot perform initial docking until cooling is complete')
    ...
    while len(cool0_Es_MM)<self.params['dock']['seeds_per_state']:
      ...
      (cool0_confs, cool0_Es_MM) = self._load_cool0()
    print "  using %d configurations from the first state of cooling"%len(cool0_Es_MM)
    ...
    lambda_o = {'T':T_HIGH, 'MM':True, 'site':True, 'sLJr_crossed':False, 'crossed':False}
    for scalable in self._scalables:
      lambda_o[scalable] = 0
