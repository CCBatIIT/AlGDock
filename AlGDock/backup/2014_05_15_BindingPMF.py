...
# TO DO: Set up checkpoints for use in open science grid.
...
last modified {2}
    ...
    if args['new_dock']['receptor_Gas'] is not None:
      self.params['dock']['receptor_Gas'] = args['new_dock']['receptor_Gas']
    if args['new_dock']['receptor_GBSA'] is not None:
      self.params['dock']['receptor_GBSA'] = args['new_dock']['receptor_GBSA']
  ...
  def _setup_universe(self, do_dock=True):
    ...
    self.molecule.heavy_atoms = [ind for (a,ind) in zip(self.molecule.atoms,range(self.molecule.numberOfAtoms())) if a.type.name!='hydrogen']
    ...
    if self._FNs['inpcrd']['R'] is not None:
      if isfile(self._FNs['inpcrd']['L']):
        lig_crd = self._read_crd(self._FNs['inpcrd']['L'],split=False)
      self.confs['receptor'] = self._read_crd(self._FNs['inpcrd']['R'],split=False)
    ...
    if self.params['dock']['rmsd'] is not False:
      if self.params['dock']['rmsd'] is True:
        ...
        rmsd_crd = self._read_crd(self.params['dock']['rmsd'])
  ...
  def initial_cool(self):
    ...
    while (not self.cool_protocol[-1]['crossed']):
      ...
      lambda_n = {'T':T, 'MM':True, 'crossed':crossed}
      ...
      if mean_acc<self.params['cool']['min_repX_acc']:
        ...
        self.confs['cool_replicas'][-1] = confs[np.argmin(Es_MM)]
        self.cool_protocol.pop()
    ...
    self._write_crd(join(self.dir['cool'],'cool_replicas.crd.gz'),
      self.confs['cool_replicas'], title='cool_replicas,cycle=-1')
  ...
  def calc_f_L(self, readOnly=False, redoSolvation=False):
    """
    ...
    for c in range(len(self.f_L_solvation), self._cool_cycle):
      ...
      u_GBSA_F = np.concatenate(\
        [self.cool_Es[K-1][c]['LGBSA'][:,-1] for c in range(fromCycle,toCycle)])
      u_Gas_F = np.concatenate(\
        [self.cool_Es[K-1][c]['LGas'][:,-1] for c in range(fromCycle,toCycle)])
      du_F = (u_GBSA_F - u_Gas_F)/RT_TARGET
      min_du_F = min(du_F)
      
      if not hasattr(self,'LGBSAsampl_Es'):
        f_L_solvation = -np.log(np.exp(-du_F+min_du_F).mean()) + min_du_F
      else:
        u_GBSA_R = np.concatenate(\
          [self.LGBSAsampl_Es[c]['GBSA'] for c in range(fromCycle,toCycle)])
        u_Gas_R = np.concatenate(\
          [self.LGBSAsampl_Es[c]['Gas'] for c in range(fromCycle,toCycle)])
        du_R = (u_Gas_R-u_GBSA_R)/RT_TARGET
#        min_du_R = min(du_R)
#        np.log(np.exp(-du_R+min_du_R).mean()) - min_du_R
        import pymbar
        f_L_solvation = pymbar.BAR(du_F, du_R, relative_tolerance=0.000001, verbose=False, compute_uncertainty=False)
      
      self.f_L_solvation.append(f_L_solvation)
      self.tee("  calculated solvation free energy of %f RT "%(\
                  self.f_L_solvation[-1])+\
  ...
  def initial_dock(self, randomOnly=False):
    ...
    checkpoint = self._load_pkl_gz(join(self.dir['dock'],'dock_checkpoint.pkl.gz'))
    if checkpoint is None:
      # Do the random docking
      # Load the first cooling state and make sure there are enough samples
      (cool0_confs, cool0_Es_MM) = self._cool0_storage()
      if len(cool0_Es_MM)<self.params['dock']['seeds_per_state']:
        raise Exception("More samples from high temperature ligand simulation needed")

      self.tee("\n>>> Initial docking")

      # Store docking parameters
      self.store_params('dock')
      ...
      self.tee("  %d ligand configurations "%len(cool0_Es_MM) + \
               ...
               "in %f s"%(time.time()-dock_start_time))
      ...
      self.tee('  raveled energy terms in %f s'%(\
        time.time()-ravel_start_time))

      stats_start_time = time.time()
      if self.params['dock']['do_calc_random_dock_stats']:
        self.calc_random_dock_stats(E)
        self.tee('  calculated random dock stats in %f s'%(\
          time.time()-stats_start_time))
    ...
    # simulate
    rejectStage = 0
    while (checkpoint is not None) or (not self.dock_protocol[-1]['crossed']):
      ...
      if checkpoint is None:
        ...
        self.tee('  selected %d seeds in %f s'%(\
          len(seeds), time.time()-seed_start_time))
        
        # Save checkpoint
        self._write_pkl_gz(join(self.dir['dock'],'dock_checkpoint.pkl.gz'),\
          (self.dock_protocol,seeds,self.confs['dock_replicas'],confs,E))
    
        if randomOnly:
          self._clear_lock('dock')
          return
      else: # if checkpoint is not None
        (self.dock_protocol,seeds,self.confs['dock_replicas'],confs,E) = checkpoint
        lambda_n = self.dock_protocol[-1]
        print '  continuing initial docking from a previous instance'
        checkpoint = None
      ...
      self._set_universe_force_field(lambda_n)
      (confs, Es_tot, g) = self._initial_sim_state('dock', seeds, \
        normalize=False, doMC=True)
      
      # Store data
      self.tee('  generated %d configurations in %f s using the following parameters:'%(\
        len(confs), time.time()-sim_start_time))
      self.tee(self.dock_protocol[-1])

      # Get state energies
      E = self._calc_E(confs)
      self.tee('  initialized state in %f s'%(time.time()-state_start_time))
      ...
      if len(self.dock_protocol)>2:
        ...
        if (mean_acc<self.params['dock']['min_repX_acc']):
          ...
          # reject the previous state and restart
          self.confs['dock_replicas'][-1] = confs[np.argmin(Es_tot)]
          self.dock_protocol.pop()
    ...
    self.tee("  there are %d states in the docking process"%(\
      len(self.dock_protocol)))

    # Save data
    self._write_crd(join(self.dir['dock'], 'dock_replicas.crd.gz'), \
      self.confs['dock_replicas'], title='dock_replicas,cycle=-1')
    self._write_pkl_gz(join(self.dir['dock'],'dock_protocol.pkl.gz'),
      self.dock_protocol)

    self.tee("\nElapsed time for initial docking: %f s"%(\
      time.time()-dock_start_time))
    self._clear_lock('dock')
  ...
  def store_params(self, process='cool'):
    if process=='cool' or process=='both':
  ...
  def calc_f_RL(self, readOnly=False):
    ...
    for c in range(self._dock_cycle):
      if len(self.dock_Es[K-1][c].keys())==0:
...
#    mean_Psi = np.array([np.mean(self.Psi[c]['GBSA']) \
#      for c in range(self._dock_cycle)])
#    std_Psi = np.array([np.std(self.Psi[c]['GBSA']) \
#      for c in range(self._dock_cycle)])
#    for c in range(len(self.dock_equilibrated_cycle), self._dock_cycle):
#      nearMean = (mean_Psi - mean_Psi[c])<std_Psi[c]
#      for (item, ind) in zip(nearMean, range(len(nearMean))):
#        if item:
#          self.dock_equilibrated_cycle.append(ind)
#          break
    ...
    for c in range(len(self.f_grid), self._dock_cycle):
      ...
      for phase in ['Gas','GBSA']:
        ...
        B[phase+'_MBAR_based'] = (phase=='GBSA')*(-self.f_L_solvation[-1]) - \
      ...
      self.tee("  calculated binding PMF of %f RT using cycles %d to %d"%(\
        B['GBSA_MBAR_based'], fromCycle, toCycle-1))
      updated = True
  ...
  def view_samples(self, process='dock', state=None,
    ...
    """
    if (self._FNs['vmd'] is None) or (not isfile(self._FNs['vmd'])):
    ...
    for c in range(firstCycle,getattr(self,'_%s_cycle'%process)):
      if not isfile(join(self.dir[process],process+'%d_%d.dcd'%(state,c))):
...
'''
    ...
    if execute:
      import tempfile
      (scriptF,scriptFN) = tempfile.mkstemp()
    ...
    for scalable in self._scalables:
      if (scalable in lambda_n.keys()) and lambda_n[scalable]>0:
        ...
        if not scalable in self._forceFields.keys():
          ...
          self.tee('  %s loaded in %f s'%(grid_FN, time.time()-loading_start_time))
  ...
  def _MC_translate_rotate(self, trials=100):
    """
  ...
  def _replica_exchange(self, process):
    ...
    for sweep in range(self.params[process]['sweeps_per_cycle']):
      ...
      for k in range(K):
        ...
        if process=='dock' and state_inds[k]<7:
          E['acc_MC'][k] = self._MC_translate_rotate(100)/100.
      ...
      if process=='dock':
        ...
        # Get rmsd values
        if self.params['dock']['rmsd'] is not False:
          E['rmsd'] = np.array([np.sqrt(((confs[k][self.molecule.heavy_atoms,:] - self.confs['rmsd'])**2).sum()/self.molecule.nhatoms) for k in range(K)])
    ...
    self.tee("  generated configurations for %d replicas"%(len(confs)) + \
      " in replica exchange cycle %d"%cycle + \
      " in %f s"%(time.time()-cycle_start_time) + \
      " with %f s for MC"%(MC_time))
    ...
    self.confs[process+'_replicas'] = \
      [storage['confs'][store_indicies[-1]][inv_state_inds[-1][state]] \
       for state in range(K)]

    # Refine the protocol by inserting states between neighbors with low
      ...
      if insert_state.any():
        for k in reversed(range(K-1)):
          if insert_state[k]:
            ...
            new_conf = self.confs[process+'_replicas'][k+1]
            self.confs[process+'_replicas'].insert(k+1, new_conf)
        K = len(self.confs[process+'_replicas'])
        self.tee('  refined protocol to have %d states'%K)
        
        # Determine new protocol with equal intervals in thermodynamic length
        progress_o = np.array([self._progress(l,process) for l in lambdas])
        ...
        if process=='dock':
          lambdas = [self._lambda(progress,process) for progress in progress_n]
        elif process=='cool':
          lambdas = [self._lambda(progress,process) for progress in reversed(progress_n)]

        # Save the protocol and configurations
        setattr(self,process+'_protocol', lambdas)
        self._write_pkl_gz(join(self.dir[process],process+'_protocol.pkl.gz'),
          lambdas)
        self._write_crd(join(self.dir[process],process+'_replicas.crd.gz'), \
          self.confs[process+'_replicas'], \
          title=(process+'_replicas,cycle=%d'%cycle))
        # Reinitialize energy storage
        setattr(self, process+'_Es', [[] for state in range(K)])
        # Exit the replica exchange cycle without postprocessing
    ...
    for state in range(K):
      if self.params[process]['keep_intermediate'] or \
        ...
        self._write_crd(\
          join(self.dir[process],process+'%d_%d.crd.gz'%(state,cycle)), \
          confs, title='state=%d,cycle=%d'%(state,cycle))
    ...
    self.tee('  postprocessed data in %f s\n'%(\
  ...
  def _sim_process(self, process):
    ...
    if (process=='dock') and (self._dock_cycle==0) and \
    ...
    if (self.params[process]['repX_cycles'] is not None) and \
      ...
      self._set_lock(process)
      self.tee("Elapsed time for %d cycles of replica exchange was %f s"%(\
         (getattr(self,'_%s_cycle'%process)-start_cycle), \
         (time.time()-repEx_start_time)))
      self._clear_lock(process)

    if (process=='cool'):
      (cool0_confs, cool0_Es_MM) = self._cool0_storage(save=True)
...
#      K = len(self.cool_protocol)
#      cool_conf = self._read_crd(join(self.dir['cool'],'cool%d_0.crd.gz'%(K-1)))[0]
#      start_conf_FN = join(self.dir['cool'],'LGBSA_start.crd')
#      self._write_crd(start_conf_FN, \
#        [cool_conf[self.molecule.prmtop_atom_order,:]], \
#        title='LGBSA_start')

    # Run GBSA of the ligand in the target state
    # TO DO: Integrate this into replica exchange?
    if (process=='cool'):
      LGBSAsampl_Es_FN = join(self.dir['cool'],'LGBSAsampl_Es.pkl.gz')
      import AlGDock.NAMD
      NAMDSim = AlGDock.NAMD.NAMD(
        prmtop=self._FNs['prmtop']['L'], inpcrd=self._FNs['inpcrd']['L'],
        fixed=None, solvent='GBSA', useCutoff=True,
        namd_command=self._FNs['namd'])
        
      if os.path.isfile(LGBSAsampl_Es_FN):
        self.LGBSAsampl_Es = self._load_pkl_gz(LGBSAsampl_Es_FN)
      elif not hasattr(self,'LGBSAsampl_Es'):
        self.LGBSAsampl_Es = []
      
      for c in range(len(self.LGBSAsampl_Es),self.params['cool']['repX_cycles']):
        outputname = join(self.dir['cool'],'LGBSAsampl%d'%(c))
        if c>0:
          inputname = join(self.dir['cool'],'LGBSAsampl%d'%(c-1))
          NAMDSim.bincoordinates = inputname+'.coor'
          NAMDSim.binvelocities = inputname+'.vel'
        EGBSA = np.array(NAMDSim.simulate(outputname, \
          steps=self.params['cool']['steps_per_seed']*\
                self.params['cool']['sweeps_per_cycle'],
          energyFields=[1, 2, 3, 4, 5, 6, 8, 12],
          keepCoor=True)[1:], dtype=float)*MMTK.Units.kcal/MMTK.Units.mol
        if NAMDSim.bincoordinates is not None:
          os.remove(NAMDSim.bincoordinates)
        if NAMDSim.binvelocities is not None:
          os.remove(NAMDSim.binvelocities)
        EGas = self._NAMD_Energy(None, 'L', 'Gas', \
          outputname+'.dcd', outputname+'pp', debug=False)
        os.remove(outputname+'.dcd')
        self.LGBSAsampl_Es.append({'GBSA':EGBSA[:,-1], 'Gas':EGas[:,-1]})
        self._write_pkl_gz(LGBSAsampl_Es_FN, self.LGBSAsampl_Es)

      self.calc_f_L(redoSolvation=True)
  ...
  def _get_dock_score_confs(self):
    ...
    The final results are stored in self.confs['dock_replicas'].
    """
    self._set_lock('dock')
    unique_confs = []
    unique_conf_Es = {}
    if self.params['dock']['score'] is True:
      ...
      self.confs['dock_replicas'] = [self.confs['ligand'] \
      ...
      if self.params['dock']['score'].endswith('.mol2') or \
        ...
        (confs_mol2,E_mol2) = self._read_dock6(self.params['dock']['score'])
        for term in E_mol2.keys():
          unique_conf_Es[term] = []
        # Ensure ligand is in the binding site
        self._set_universe_force_field({'site':True,'T':T_TARGET})
        for ind in range(len(confs_mol2)):
          self.universe.setConfiguration(Configuration(self.universe, confs_mol2[ind]))
          if self.universe.energy()<1.:
            unique_confs.append(confs_mol2[ind])
            for term in E_mol2.keys():
              unique_conf_Es[term].append(E_mol2[term][ind])
            if hasattr(self,'dock_protocol') and \
              (len(unique_confs)==len(self.dock_protocol)):
              break
        for term in E_mol2.keys():
          unique_conf_Es[term] = np.array(unique_conf_Es[term])
        count['dock6'] = len(unique_confs)
        confs = [c for c in reversed(unique_confs)]
        if ('dock_replicas' in self.confs.keys()) and \
           (len(confs)<len(self.dock_protocol)):
          confs = self.confs['dock_replicas'] + confs
          count['initial_dock'] = len(self.confs['dock_replicas'])
      else:
        raise Exception('Unrecognized file type for configurations')
      if len(confs)==0:
        raise Exception('None of the docked configurations are in the binding site')
      self.confs['dock_replicas'] = confs[-len(self.dock_protocol):]
      while len(self.confs['dock_replicas'])<len(self.dock_protocol):
        self.confs['dock_replicas'].append(self.confs['dock_replicas'][-1])
        count['duplicated'] += 1
    ...
    self._clear_lock('dock')
    return (unique_confs,unique_conf_Es)
  ...
  def _next_dock_state(self, E=None, lambda_o=None, pow=None):
    ...
    if self.params['dock']['protocol']=='Set':
      ...
      if tL_tensor>0:
        ...
        progress = min(lambda_o['LJr'] + dL, 1)
        return self._lambda(progress)
        if progress == 1:
  ...
  def _tL_tensor(self, E, lambda_c, process='dock'):
    ...
    if process=='dock':
      ...
      progress = lambda_c['LJr']
      Psi_g = self._u_kln([E],
        [{'LJr':1,'LJa':1,'ELE':1}], noBeta=True)
      U_RL_g = self._u_kln([E],
        [{'MM':True,'site':True,'T':T,\
        'LJr':progress,'LJa':progress,'ELE':progress}], noBeta=True)
      return Psi_g.std()/(R*T) + \
  ...
  def _progress(self, lambda_c, process='dock'):
    if process=='dock':
      return lambda_c['LJr']
    elif process=='cool':
      return lambda_c['T']
  ...
  def _lambda(self, progress, process='dock', lambda_o=None):
    if process=='dock':
      if lambda_o is None:
        lambda_o = self.dock_protocol[-1]
      lambda_n = copy.deepcopy(lambda_o)
      lambda_n['LJr'] = progress
      lambda_n['LJa'] = progress
      lambda_n['ELE'] = progress
      lambda_n['T'] = progress*(T_TARGET-T_HIGH) + T_HIGH
      lambda_n['crossed'] = (progress == 1)
    elif process=='cool':
      if lambda_o is None:
        lambda_o = self.cool_protocol[-1]
      lambda_n = copy.deepcopy(lambda_o)
      lambda_n['T'] = progress
      lambda_n['crossed'] = (abs(progress-T_TARGET)<0.01)
    else:
      raise Exception("Unknown process!")
    return lambda_n
  ...
  def _postprocess(self,
    ...
    # Save progress
    for (p,c) in updated:
      self.tee("  postprocessed {0}ing cycle {1} with NAMD".format(p,c))
      if p=='cool' or p=='dock':
        K = len(getattr(self,p+'_protocol'))
        energyFN = join(self.dir[p], '%s_Es_%d.pkl.gz'%(p, c))
        energies = [getattr(self, p+'_Es')[state][c] for state in range(K)]
  ...
  def _cool0_storage(self, save=False):
    cool0_store_FN = join(self.dir['cool'],'cool0.crd_Es.gz')
    if save or (not isfile(cool0_store_FN)):
      # Load configurations
      confs = []
      c = 0
      while isfile(join(self.dir['cool'],'cool0_%d.crd.gz'%c)):
      ...
      if len(E_MM)!=len(confs):
        print '\nCooling state energies:'
        print E_MM
        print 'There are %d configurations'%len(confs)
      ...
      dat = ([confs[ind] for ind in random_dock_inds], \
              [E_MM[ind]  for ind in random_dock_inds])
      self._write_pkl_gz(cool0_store_FN, dat)
      return dat
    else:
      return self._load_pkl_gz(cool0_store_FN)

  def _read_crd(self, crdFN, split=True, return_title=False):
    """ 
    Reads an AMBER or VMD format coordinate file.
    """
    if not isfile(crdFN):
      raise Exception('Coordinate file %s does not exist!'%crdFN)
    if crdFN.endswith('.gz'):
  ...
  def _write_crd(self, crdFN, crd, title='', appendIfExists=False, vmdStyle=False):
    ...
    """
    if (appendIfExists and isfile(crdFN)):
      if crdFN.endswith('.gz'):
    ...
    crdF.close()
    self.tee("  wrote %d configurations to %s"%(len(crd), crdFN))
  ...
  def _load_progress(self, p):
    ...
    if getattr(self, p+'_protocol') is None:
      ...
      print '  %s process has not yet started initialization'%p
      return
    elif not (getattr(self, p+'_protocol')[-1]['crossed']):
      print getattr(self, p+'_protocol')
      setattr(self, '_%s_cycle'%p, 0)
      print '  %s process has not yet finished initialization'%p
      return

    # Get the current cycle based on the existing energy files
    # (this may be superceded by the value in the replica exchange file)
    cycle = 0
    while isfile(join(self.dir[p], '%s_Es_%d.pkl.gz'%(p, cycle))):
      cycle += 1

    # Load replicas for replica exchange
    if not p+'_replicas' in self.confs.keys():
      replica_FN = join(self.dir[p], p+'_replicas.crd.gz')
      if isfile(replica_FN):
        (confs, title) = self._read_crd(replica_FN, return_title=True)
        self.confs[p+'_replicas'] = confs
        if title.find('=')>-1:
          cycle = int(title.split('=')[-1]) + 1

    print '  %s process is in cycle %d'%(p,cycle)
    setattr(self, '_%s_cycle'%p, cycle)

    # Load energies in each cycle
    if (not hasattr(self, p+'_Es')) or (getattr(self, p+'_Es')==[]):
      K = len(getattr(self, p+'_protocol'))
      setattr(self, p+'_Es', [[] for state in range(K)])

      for c in range(getattr(self,'_%s_cycle'%p)):
        Es_FN = join(self.dir[p], '%s_Es_%d.pkl.gz'%(p, c))
        if isfile(Es_FN):
          cycle_Es = self._load_pkl_gz(Es_FN)
        else:
          cycle_Es = [{} for state in range(K)]
...
if __name__ == '__main__':
  ...
  parser.add_argument('--run_type',
    choices=['pose_energies','initial_cool','cool','random_dock',\
             'initial_dock','dock','all', \
             'store_params','free_energies', 'redo_postprocess', None],
  ...
  parser.add_argument('--min_repX_acc', type=float, default=0.5,
