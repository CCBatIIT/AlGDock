  ...
  def initial_dock(self, randomOnly=False):
    ...
    while (not self.dock_protocol[-1]['crossed']):
      ...
      self.tee("  generated %d configurations "%len(confs) + \
               ...
               "in %f s "%(time.time()-sim_start_time))
...
'''
  ...
  def _replica_exchange(self, process):
    ...
    # There will be at least 10 and up to sweeps_per_cycle saved samples
    # max(int(np.ceil(1+2*tau_ac)),1) is the minimum stride due to autocorrelation
    # max(self.params['dock']['sweeps_per_cycle']/10,1)
    # is the maximum stride to have 10 samples, if possible
    stride = min(max(int(np.ceil(1+2*tau_ac)),1), \
                 max(self.params['dock']['sweeps_per_cycle']/10,1))
