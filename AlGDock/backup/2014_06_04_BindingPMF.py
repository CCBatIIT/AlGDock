...
last modified {2}
  ...
  def initial_dock(self, randomOnly=False):
    ...
    if self.dock_protocol==[]:
      ...
      confs = self.confs['dock']['samples'][-1]
      E = self.dock_Es[-1]
    ...
    self.tee("  %d states in the docking process sampled in %f s"%(K,\
...
'''
    ...
    if insert_state.any():
      for k in reversed(range(K-1)):
        if insert_state[k]:
          ...
          new_conf = self.confs[process]['replicas'][k+1]
  ...
  def _get_dock_score_confs(self):
    ...
    if self.params['dock']['score'] is True:
      ...
      self.confs['dock']['replicas'] = [self.confs['ligand'] \
      ...
      if self.params['dock']['score'].endswith('.mol2') or \
        ...
        if ('dock_replicas' in self.confs.keys()) and \
          ...
          confs = self.confs['dock']['replicas'] + confs
          count['initial_dock'] = len(self.confs['dock']['replicas'])
      else:
      ...
      self.confs['dock']['replicas'] = confs[-len(self.dock_protocol):]
...
''')
  ...
  def _load_progress(self, p):
    ...
    if isfile(progress_FN):
      try:
        progress = self._load_pkl_gz(progress_FN)
      except:
        progress = None
        print '  failed to load '+progress_FN
      if (progress is None) and isfile(progress_FN+'.BAK'):
        print'  loading backup'
        try:
          progress = self._load_pkl_gz(progress_FN+'.BAK')
        except:
          print 'Failed to load '+progress_FN+'.BAK'
          progress = None
