...
# TO DO: Use a logger http://docs.python.org/2/library/logging.html

# TO DO: Additional MC moves in docking,
# especially at high T and lower interaction strengths.
# Trying an external translation and rotation move

# TO DO: Fourier transform docking for initial stage.
...
Molecular docking with adaptively scaled alchemical interaction grids
  ...
  def initial_dock(self):
    ...
    # TO DO: Add checkpoints to this stage
    ...
    while not self.dock_protocol[-1]['crossed']:
      ...
      print '  appended to protocol in %f s'%(time.time()-protocol_start_time)
      ...
      print '  generated %d configurations in %f s using the following parameters:'%(\
      ...
      print '  calculated energies in %f s'%(time.time()-energy_start_time)
    ...
    print '  wrote configurations and energies in %f s'%(time.time()-write_start_time)
...
'''
  ...
  def _replica_exchange(self, process):
    ...
    if process=='cool':
      ...
      terms = ['MM','site','misc','acc_MC'] + self._scalables
