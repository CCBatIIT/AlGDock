...
# 3/21 - Added code to check replica exchange rate during initialization phases
# TO DO: Make the minimum replica exchange rate a parameter, not hard coded
# TO DO: Protocol refinement based on thermodynamic length
...
Molecular docking with adaptively scaled alchemical interaction grids
    ...
    for FN in [self._FNs['prmtop']['RL'],
      ...
      if (FN is None) or (not isfile(FN)):
        ...
        if (hasattr(args,'run_type') and \
           (args.run_type is not None) and \
           (args.run_type in ['random_dock','initial_dock','dock'])):
  ...
  def initial_cool(self):
    ...
    std_Es_MM = Es_MM.std()
    sigma_dl_a = std_Es_MM/(R*T*T) # Metric tensor for the thermodynamic length
    ...
    while (not self.cool_protocol[-1]['crossed']):
      ...
      crossed = False
      if sigma_dl_a>0:
        T = To - self.params['cool']['therm_speed']/sigma_dl_a
        if T < T_TARGET:
          T = T_TARGET
          crossed = True
      else:
      ...
      self.tee("  the estimated replica exchange acceptance rate between the previous and new state is %f"%mean_acc)

      # Estimate the metric tensor for the thermodynamic length
      std_Es_MM = Es_MM.std()
      sigma_dl_a2 = (std_Es_MM_o/(R*To*To) + std_Es_MM/(R*T*T))/2
      self.tee("  the estimated metric tensor for thermodynamic length " + \
               "was %f with one state and %f with two"%(sigma_dl_a,sigma_dl_a2))
      ...
      if mean_acc<0.5:
        ...
        confs = confs_o
        Es_MM = Es_MM_o
        std_Es_MM = std_Es_MM_o
        if sigma_dl_a2<sigma_dl_a:
          sigma_dl_a = sigma_dl_a*1.25
        else:
          sigma_dl_a = sigma_dl_a2
        self.tee("  state rejected!\n")
      else:
        self.tee("")
    ...
    self._write_crd(join(self.dir['cool'],'cool_replicas.crd.gz'),
      self.confs['cool_replicas'], title='cool_replicas,cycle=0')
  ...
  def calc_f_L(self, readOnly=False):
    ...
    for c in range(len(self.f_L_solvation), self._cool_cycle):
      ...
      self.tee("  calculated cooling free energy of %f kT "%(self.f_cool[-1][-1])+\
  ...
  def initial_dock(self, randomOnly=False):
    ...
    if checkpoint is None:
      ...
      while len(cool0_Es_MM)<self.params['dock']['seeds_per_state']:
        ...
        (cool0_confs, cool0_Es_MM) = self._load_cool0()
      self.tee("  using %d configurations "%len(cool0_Es_MM) +\
    ...
    while not self.dock_protocol[-1]['crossed']:
      ...
      if checkpoint is None:
        if rejectStage==0:
          ...
          lambda_n = self._next_dock_state(E = E, En = E_r, \
            lambda_o = lambda_o, lambda_n = lambda_r, pow = rejectStage)
        self.dock_protocol.append(lambda_n)
        
        # Use free energy perturbation to estimate the free energy difference between
        # the first and second docking state
  ...
  def calc_f_RL(self, readOnly=False):
    ...
    for c in range(len(self.f_grid), self._dock_cycle):
      ...
      self.tee("  calculated binding PMF of %f kT using cycles %d to %d"%(\
  ...
  def calc_RMSD(self, rmsd_ref=None):
    ...
    for c in range(self._dock_cycle):
      ...
      if not ref_FN in self.rmsd[c].keys():
        crdFN = join(self.dir['dock'],'dock%d_%d.crd.gz'%(state,c))
        if not isfile(crdFN):
          break
        crd = self._read_crd(crdFN, split=True)
...
file delete '''+dcdFN+'\n')
      ...
      subprocess.call(\
        [self._FNs['vmd'], '-nt', '-dispdev', 'text', '-e', scriptF.name],\
        stdout=vmd_out)
      vmd_out.close()
      ...
      for c in range(self._dock_cycle):
        if not ref_FN in self.rmsd[c].keys():
          temp_FN = join(self.dir['dock'],'rmsd%d_%d.txt'%(state,c))
          if isfile(temp_FN):
...
'''
  ...
  def _replica_exchange(self, process):
    ...
    if cycle==0:
      ...
      # Insert new states between neighbors with low exchange probabilities
      insert_state = (mean_acc<0.5)
      if insert_state.any():
        self.tee('  refining protocol to increase '+\
                 'mean replica exchange acceptance rate')
        for k in reversed(range(K-1)):
          if insert_state[k]:
            # Define the state
            new_lambda = {}
            for key in lambdas[k+1].keys():
              if key in lambdas[k].keys():
                if isinstance(lambdas[k+1][key],float):
                  new_lambda[key] = (lambdas[k][key] + lambdas[k+1][key])/2
                else:
                  new_lambda[key] = lambdas[k][key]
            # Inserting the state into the protocol
            self.tee('  inserted state:')
            self.tee(new_lambda)
            lambdas.insert(k+1, new_lambda)
        ...
        K = len(lambdas)
        self.tee('  new protocol has %d states\n'%K)
  ...
  def _next_dock_state(self, E=None, En=None, \
      lambda_o=None, lambda_n=None, pow=None):
    """
    ...
    if self.params['dock']['protocol']=='Set':
      ...
      if self.params['dock']['legs']==1:
        ...
        sigma_dl_0 = calc_sigma_dl(E,lambda_o['LJr'],lambda_o['T'])
        sigma_dl = sigma_dl_0
        if (En is not None) and (lambda_n is not None):
          sigma_dl_1 = calc_sigma_dl(En,lambda_n['LJr'],lambda_n['T'])
          sigma_dl_2 = (sigma_dl_0 + sigma_dl_1)/2
          if sigma_dl_2<sigma_dl_0:
            sigma_dl = sigma_dl_0*(1.25**pow)
          else:
            sigma_dl = sigma_dl_2
          self.tee("  Thermodynamic length metric tensor 1: %f, 2: %f, used: %f"%(sigma_dl_0,sigma_dl_1,sigma_dl))
        if sigma_dl>0:
          dL = self.params['dock']['therm_speed']/sigma_dl
    ...
    for (p, state, cycle, moiety) in conditions:
      ...
      for c in cycles:
        ...
        for phase in phases:
          ...
          # Skip NAMD
          # if the function is being run in readOnly mode,
          # if the coordinate file does not exist, or
          # if the energies are already in memory.
          if readOnly or \
...
if __name__ == '__main__':
  ...
  parser.add_argument('--run_type',
    choices=['initial_cool','cool','random_dock','initial_dock','dock','all'],
