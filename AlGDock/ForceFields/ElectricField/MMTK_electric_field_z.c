     ...
     */
{
  ...
  for (atom_index = 0; atom_index < natoms; atom_index++) {
    ...
    double e = q*(ez*coordinates[atom_index][2]);
...
static PyMethodDef functions[] = {
  {"ElectricFieldTerm_z", ElectricFieldTerm_z, 1},
  {NULL, NULL}		/* sentinel */
};
...
DL_EXPORT(void)
initMMTK_electric_field_z(void)
{
  PyObject *m;

  /* Create the module and add the functions. */
  m = Py_InitModule("MMTK_electric_field_z", functions);
  ...
  /* Check for errors. */
  if (PyErr_Occurred())
    Py_FatalError("can't initialize module MMTK_electric_field_z");
}
