...
from MMTK_CatmullSpline_grid import CatmullSplineGridTerm

# Exactly the same as in the pure python version
class CatmullSplineGridForceField(ForceField):
    """
    Force field based on CatmullSpline interpolation between points on the 3D grid
    """

    def __init__(self, FN, strength, scaling_property, scaling_prefactor=None,
      grid_name='CatmullSpline grid', max_val=-1.0):
        """
        ...
        if not (self.grid_data['origin']==0.0).all():
          raise Exception('CatmullSpline grid origin in %s not at (0, 0, 0)!'%self.FN)
    ...
    def evaluatorTerms(self, universe, subset1, subset2, global_data):
        ...
        return [CatmullSplineGridTerm(universe, \
