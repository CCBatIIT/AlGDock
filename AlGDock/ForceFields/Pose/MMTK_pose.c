...
static PyObject *
PoseExtDistTerm(PyObject *dummy, PyObject *args)
{
  PyFFEnergyTermObject *self;
  char *name = "pose external distance";
  self = PyFFEnergyTerm_New();
  if (self == NULL)
    return NULL;
  if (!PyArg_ParseTuple(args, "O!OOO|s",
                        &PyUniverseSpec_Type, &self->universe_spec,
                        &self->data[0], &self->data[1], &self->data[2],
                        &name))
    return NULL;
  self->evaluator_name = "pose external distance";
  self->term_names[0] = allocstring(name);
  if (self->term_names[0] == NULL)
    return PyErr_NoMemory();
  Py_INCREF(self->universe_spec);
...
static PyObject *
PoseExtAnglTerm(PyObject *dummy, PyObject *args)
{
  PyFFEnergyTermObject *self;
  char *name = "pose external angle";
  self = PyFFEnergyTerm_New();
  if (self == NULL)
    return NULL;
  if (!PyArg_ParseTuple(args, "O!OOO|s",
                        ...
                        &name))
    return NULL;
  self->evaluator_name = "pose external angle";
  self->term_names[0] = allocstring(name);
  if (self->term_names[0] == NULL)
    return PyErr_NoMemory();
  Py_INCREF(self->universe_spec);
...
static PyObject *
PoseExtDiheTerm(PyObject *dummy, PyObject *args)
{
  PyFFEnergyTermObject *self;
