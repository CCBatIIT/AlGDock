...
class Topology:
  ...
  def __init__(self, args, includeReceptor=False):
    ...
    if includeReceptor:
      if (args.FNs['prmtop']['R'] is not None) and \
        ...
        ligand_ind = [
          ...
          if prmtop_RL['RESIDUE_LABEL'][ind] not in prmtop_R['RESIDUE_LABEL'] and prmtop_RL['RESIDUE_LABEL'][ind] != 'HIE'
        ]
        print 'ligand_ind', ligand_ind
        if len(ligand_ind) == 0:
          raise Exception('Ligand not found in complex prmtop')
          # self.L_first_atom = 0 # BANDAID
        elif len(ligand_ind) > 1:
          print '  possible ligand residue labels: '+ ', '.join([prmtop_RL['RESIDUE_LABEL'][ind] for ind in ligand_ind])
          
        print 'ligand residue name: ' + prmtop_RL['RESIDUE_LABEL'][ligand_ind[0]].strip()
    ...
    print 'L_first_atom', self.L_first_atom
    assert hasattr(self, 'L_first_atom')
