...
#  restart_data = ['configuration', 'velocities', 'energy']
  ...
  cdef start(NUTSIntegrator self):
    ...
    cdef int natoms, nsteps, total_steps
    cdef Py_ssize_t i_atm, i_dim

    cdef float joint, logu, e_next
    cdef N.ndarray[double, ndim=2] xminus, xplus, vminus, vplus, x_next
    cdef int j, n, steps
    
    cdef float alpha, alpha_tot
    cdef int nalpha, nalpha_tot
    ...
    while total_steps < nsteps:
      ...
      joint = -(self.energy.energy + ke)/self.RT
      ...
      x_next = N.copy(self.x)
      e_next = self.energy.energy
      ...
      while (s==1):
        ...
        if N.random.rand()<0.5:
          ...
          (xminus, vminus, NA0, NA1, 
           ...
           alpha, nalpha) = \
            self.build_tree(logu, j, -1*self.getOption('delta_t'), steps, joint)
        else:
          # Forward
          self.x = xplus
          self.v = vplus
          ...
          (NA0, NA1, xplus, vplus, 
           xprime, eprime, nprime, sprime, steps,
           alpha, nalpha) = \
            self.build_tree(logu, j, self.getOption('delta_t'), steps, joint)
        ...
        s = sprime and self.stop_criterion(xminus, xplus, vminus, vplus)
        # Increment depth
        j += 1

      if normalize:
        self.universe.setConfiguration(Configuration(self.universe, x_next))
        self.universe.normalizePosition()
        x_next = N.copy(self.universe.configuration().array)
    ...
    self.universe.setConfiguration(Configuration(self.universe, x_next))
    return (xs, energies, (alpha_tot/nalpha_tot), delta_t)
